...
int32_t I2cFreeMutex(void){
    ...
    // Release the mutex
    if (xSemaphoreGive(sensorI2cMutexHandle) != pdPASS){
        // Failed to release the mutex
        error = ERROR_NOT_READY;  // replace with an appropriate error code
    }
...
int32_t I2cGetMutex(TickType_t waitTime){
    ...
    if (xSemaphoreTake(sensorI2cMutexHandle, waitTime) != pdPASS){
        // Failed to acquire the mutex within the specified wait time
...
int32_t I2cReadDataWait(I2C_Data *data, const TickType_t delay, const TickType_t xMaxBlockTime){
	...
	if( xSemaphoreTake( semHandle, xMaxBlockTime ) == pdTRUE ){
		...
		/* The call to xSemaphoreTake() timed out after reading. */
		error = ERR_TIMEOUT;
		goto exitError0;
	}
