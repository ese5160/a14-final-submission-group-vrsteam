...
if(f_open(&file_object, (char const *)testA_file_name, FA_READ) == FR_OK)	FlagA = true;
f_close(&file_object);

if(FlagA){
if(!readFirmwareFromSDCard(testA_bin_file))
//SerialConsoleWriteString("NVM write failed!");
SerialConsoleWriteString("NVM write succeed!\r\n");
f_unlink(testA_file_name);
}
else
{
if(!readFirmwareFromSDCard(testB_bin_file))
//SerialConsoleWriteString("NVM write failed!");
SerialConsoleWriteString("NVM write succeed!\r\n");
